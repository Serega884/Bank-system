...
Для запуска тестов необходимо запустить файл project\cypress\e2e\test1.cy.js.
Первый тест происходит авторизация.
Второй тест проверяет есть ли список с счетами, по умолчанию у нас один счет есть, поэтому проверяю на наличие класса, который находится в карточке для создания счета.
Третий тест: происходит нажатие на кнопку открыть на первом счете, мы проверяем что страница открыта и на ней есть надпись "Динамика баланса".
Четвертый тест проверяет возможность перевода со счета на счет, вводится номер счета куду отправляются деньги, сумма, при нажатии на кнопку "Отправить" таблице проверяется сумма которая должна была отправлена. 