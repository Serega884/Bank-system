...
const ViewController = (() => {
  ...
  return {
    ...
    setupTableStyle: (data) => {
      ...
      if (!table.classList.contains(data.navStyle)) {
        table.classList.add(data.navStyle);
      }
    },
    ...
    setupPaginationNav: (data) => {
      ...
      if (navDom) {
        navDom.parentElement.removeChild(navDom);
      }
    ...
    setupPagesInNav: (data, start, end) => {
      ...
      let pgNav = document.querySelector(
        `.paginator-${data.tableID} .pg-nav-numbers`
      );
      if (pgNav) {
        pgNav.parentElement.removeChild(pgNav);
      }

      //create new pagination nav
      let navBlock = document.querySelector(
        `.paginator-${data.tableID} .nav-pages-container`
      );
    ...
    addNavActiveStyling: (data, pageNum) => {
      ...
      let pgEl;
      pgNav
        .querySelectorAll(`.paginator-${data.tableID} [data-page]`)
        .forEach((el) => {
...
export const Paginator = ((modelCtrl, viewCtrl) => {
  ...
  let initDataset = (data) => {
    ...
    document.querySelector(`#${data.tableID} tbody`)
      ? (data.domSelector = `#${data.tableID} tbody`)
      : (data.domSelector = data.tableID);
    data.page = 1;
    data.rows ? (data.rows = data.rows) : (data.rows = 25);
    data.navRange ? (data.navRange = data.navRange) : (data.navRange = 5);
    data.navStyle
      ? (data.navStyle = data.navStyle)
  ...
  let setupPaginatedTable = (id) => {
    ...
    if (table.clickEvents) {
      attachCustomListeners(table);
    }
  };
  ...
  let setupNavBlockWithBtns = (id) => {
    ...
    document
      .querySelectorAll(`.paginator-${table.tableID} [data-dir]`)
      .forEach((el) => {
  ...
  let getPageNavPos = (id, pageNum) => {
    ...
    let navArr = [];
    document
      .querySelectorAll(`.paginator-${table.tableID} [data-page]`)
      .forEach((el) => {
